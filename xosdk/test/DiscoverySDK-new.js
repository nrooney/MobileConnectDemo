...
function discoveryGetFunction(url, authorization, mcc, mnc, msisdn, ipAddress, redirectUri, callbackFunction, followRedirect){
	var parameters = url;
	...
	if(!!mcc && !!mnc) {
		parameters = addParameter(parameters, 'Identified-MCC', mcc);
		parameters = addParameter(parameters, 'Identified-MNC', mnc);
	}
			...
			if(xhr.status===200){
				...
				if(followRedirect && !!result["operatorSelection"]){
					selectOperator(result, function(mcc, mnc){
						if(mcc != null && mcc >= 0){
							discoveryGetFunction(url, authorization, mcci, mnc, msisdn, ipAddress, redirectUri, callbackFunction, false);
						}else{
				...
				if(_retries<10){
					setTimeout(function(){
						_retries++;
						discoveryGetFunction(authorization, mcci, mnc, accept, ipAddress);
					}, 1000);
				}else{
...
function getDiscoveryPassive(serviceUri, consumerKey, consumerSecret, encrypt, mcc, mnc, msisdn, sourceIP, redirectUri, callbackFunction) {
	...
	discoveryGetFunction(serviceUri,getAuthorizationToCallGetDiscovery(encrypt,authorization), mcc, mnc, msisdn, sourceIP,redirectUri, callbackFunction);
}
/**
 * Call a ‘Discovery’ API to identify the network operator.
 * The discovery API results are locally cacheable for a period of time identified by a time to live value in the response.
...
function getDiscoveryActive(serviceUri, consumerKey, consumerSecret, encrypt, mcc, mnc, msisdn, sourceIP, redirectUri, callbackFunction) {
	...
	discoveryGetFunction(serviceUri,getAuthorizationToCallGetDiscovery(encrypt,authorization), mcc, mnc, msisdn, sourceIP, redirectUri, callbackFunction, true);
}
...
function completeDiscovery(serviceUri, consumerKey, consumerSecret, encrypt, callbackFunction) {
        ...
        if (mcc && mnc) {
                discoveryGetFunction(serviceUri,getAuthorizationToCallGetDiscovery(encrypt,authorization), mcc, mnc),null, null,null, callbackFunction,false);
        } else {
...
function getMCCandMNCToCallGetDiscovery(mcc,mnc) {
	...
	if(!!mcc && !!mnc){
		mcc_mnc = mcc+"_"+mnc;
		if(!!localStorage.getObject('mcc')) 
			localStorage.removeItem('mcc');
                if(!!localStorage.getObject('mnc')) 
                        localStorage.removeItem('mnc');

		localStorage.setObject('mcc',mcc);
		localStorage.setObject('mnc',mnc);
	}else{
		getMccAndMnc();
		if(localStorage.getObject('mcc_mnc')){
